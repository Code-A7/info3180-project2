...
def get_conversations():
    ...
    received_from = (
        ...
        .filter(Message.receiver_id == user.user_id)
        .distinct()
    )
    ...
    for other_user_id in user_ids:
        ...
        if not check_match_exists(user.user_id, other_user_id):
            continue
...
def get_message_history(other_user_id):
    ...
    return (
        jsonify(
            {
                "messages": [m.to_dict_extended(user.user_id) for m in messages.items],
...
def send_message(other_user_id):
    ...
    # Create message
    message = Message(
        sender_id=user.user_id, receiver_id=other_user_id, content=content
    )
    db.session.add(message)
    db.session.commit()

    # Added TO Email
    create_notification(other_user_id, "message", "You have a new message")
    ...
    try:
        ...
        emit(
            ...
            "new_message",
            {
                ...
                "sender_name": (
                    Profile.query.filter_by(user_id=user.user_id).first().name
                    if Profile.query.filter_by(user_id=user.user_id).first()
                    else "Unknown"
                ),
                "content": content,
    ...
    return jsonify(message.to_dict_extended(user.user_id)), 201
...
def mark_message_read(message_id):
    ...
    message = Message.query.filter_by(
        message_id=message_id, receiver_id=user.user_id
    ).first()
    if not message:
...
def get_unread_count():
    ...
    count = Message.query.filter(
        Message.receiver_id == user.user_id, Message.read_at.is_(None)
    ).count()
...
def send_typing_status(other_user_id):
    ...
    try:
        ...
        emit(
            other_user_id,
            "user_typing",
            {"user_id": user.user_id, "user_name": sender_name, "is_typing": is_typing},
        )
    except Exception as e:
        print(f"WebSocket emit error: {e}")
...
def cleanup_old_messages():
    ...
    return (
        jsonify(
            {"message": f"Cleaned up {deleted} messages", "deleted_count": deleted}
        ),
        200,
    )
