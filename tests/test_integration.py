...
from app import db
from app.models import User, Profile, Match, Message, Notification
...
class TestCompleteUserFlow:
    ...
    def test_like_match_message_flow(self, client, app, match_pair, third_user_with_profile):
        ...
        response = client.post(f'/api/matches/like/{match_pair["user1"]["user_id"]}',
            headers={'Authorization': f'Bearer {third_user_with_profile["token"]}'}
        )
        assert response.status_code == 200
        assert response.json['match'] == True
    
    def test_profile_search_bookmark_flow(self, client, app, user_with_profile, second_user_with_profile, third_user_with_profile):
        """Discovery flow: search -> bookmark -> view bookmarks."""
        # Search profiles
        response = client.post('/api/matches/search',
            json={'gender': 'female'},
        ...
        response = client.post(f'/api/matches/bookmark/{second_user_with_profile["user_id"]}',
            headers={'Authorization': f'Bearer {user_with_profile["token"]}'}
        )
    ...
    def test_notification_flow(self, client, app, user_with_profile, second_user_with_profile):
...
class TestDataIsolation:
    ...
    def test_cannot_message_non_match(self, client, user_with_profile, second_user_with_profile):
    ...
    def test_cannot_bookmark_self(self, client, user_with_profile):
        ...
        response = client.post(f'/api/matches/bookmark/{user_with_profile["user_id"]}',
            headers={'Authorization': f'Bearer {user_with_profile["token"]}'}
        )
        
        assert response.status_code in [200, 400, 404]
    
    def test_cannot_access_other_users_notifications(self, client, user_with_profile, second_user_with_profile):
    ...
    def test_cannot_update_other_users_profile(self, client, user_with_profile, second_user_with_profile):
...
class TestEdgeCases:
    ...
    def test_multiple_likes_same_user(self, client, user_with_profile, second_user_with_profile):
