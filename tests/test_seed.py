...
class TestSeedScript:
    ...
    def test_seed_users_are_verified(self, app):
        ...
        with app.app_context():
            ...
            for user in users:
                assert user.is_verified == True, f"User {user.email} is not verified"
    ...
    def test_seed_users_have_valid_passwords(self, app):
        ...
        with app.app_context():
            ...
            for user in users:
                ...
                assert user.password_hash.startswith('$2'), f"Invalid password hash format for {user.email}"
                
                # Verify password works
                assert bcrypt.check_password_hash(user.password_hash, 'password123'), f"Password check failed for {user.email}"
    ...
    def test_seed_can_login_with_all_users(self, client, app):
        ...
        for i in range(1, 11):
            ...
            assert response.status_code == 200, f"Login failed for user{i}@test.com: {response.json}"
            assert 'token' in response.json
...
class TestSeededProfiles:
    ...
    def test_seed_profiles_have_names(self, app):
        ...
        with app.app_context():
            ...
            for profile in profiles:
                assert profile.name is not None, f"Profile for user {profile.user_id} has no name"
                assert len(profile.name) > 0, f"Profile name is empty for user {profile.user_id}"
    ...
    def test_seed_profiles_have_valid_ages(self, app):
        ...
        with app.app_context():
            ...
            profiles = Profile.query.all()
            for profile in profiles:
                assert 18 <= profile.age <= 100, f"Profile age {profile.age} is invalid for user {profile.user_id}"
    ...
    def test_seed_profiles_have_minimum_interests(self, app):
        ...
        with app.app_context():
            ...
            profiles = Profile.query.all()
            for profile in profiles:
                assert profile.interests is not None, f"Profile for user {profile.user_id} has no interests"
                assert len(profile.interests) >= 3, f"Profile for user {profile.user_id} has {len(profile.interests)} interests (need 3+)"
    ...
    def test_seed_profiles_have_gender(self, app):
        ...
        with app.app_context():
            ...
            profiles = Profile.query.all()
            for profile in profiles:
                assert profile.gender is not None, f"Profile for user {profile.user_id} has no gender"
    ...
    def test_seed_profiles_have_interests(self, app):
        ...
        with app.app_context():
            ...
            profiles = Profile.query.all()
            for profile in profiles:
                assert isinstance(profile.interests, list), f"Interests should be a list for user {profile.user_id}"
        ...
        with app.app_context():
            ...
            for match in matches:
                ...
                like1 = Like.query.filter_by(
                    from_user_id=match.user1_id,
                    to_user_id=match.user2_id
                ).first()
                
                like2 = Like.query.filter_by(
                    from_user_id=match.user2_id,
                    to_user_id=match.user1_id
                ).first()
                
                assert like1 is not None, f"Like from user {match.user1_id} to {match.user2_id} not found"
                assert like2 is not None, f"Like from user {match.user2_id} to {match.user1_id} not found"
...
class TestSeededMessages:
    ...
    def test_seed_messages_have_valid_users(self, app):
        ...
        with app.app_context():
            ...
            for message in messages:
                ...
                assert sender is not None, f"Message has invalid sender: {message.sender_id}"
                assert receiver is not None, f"Message has invalid receiver: {message.receiver_id}"
    ...
    def test_seed_messages_have_content(self, app):
        ...
        with app.app_context():
            ...
            messages = Message.query.all()
            for message in messages:
                assert message.content is not None, f"Message {message.message_id} has no content"
                assert len(message.content) > 0, f"Message {message.message_id} has empty content"
...
class TestSeededNotifications:
    ...
    def test_seed_notifications_have_messages(self, app):
        ...
        with app.app_context():
            ...
            for notification in notifications:
                assert notification.message is not None, f"Notification {notification.notification_id} has no message"
                assert len(notification.message) > 0, f"Notification {notification.notification_id} has empty message"
    ...
    def test_seed_notifications_have_valid_types(self, app):
        ...
        with app.app_context():
            ...
            notifications = Notification.query.all()
            for notification in notifications:
                assert notification.type in ['match', 'like', 'message'], f"Invalid notification type: {notification.type}"
...
class TestSeededBookmarks:
    ...
    def test_seed_bookmarks_have_valid_users(self, app):
        ...
        with app.app_context():
            ...
            for bookmark in bookmarks:
                ...
                assert user is not None, f"Bookmark has invalid user: {bookmark.user_id}"
                assert bookmarked_user is not None, f"Bookmark has invalid bookmarked user: {bookmark.bookmarked_user_id}"
    ...
    def test_seed_no_self_bookmarks(self, app):
        ...
        with app.app_context():
            ...
            bookmarks = Bookmark.query.all()
            for bookmark in bookmarks:
                assert bookmark.user_id != bookmark.bookmarked_user_id, f"User {bookmark.user_id} bookmarked themselves"
