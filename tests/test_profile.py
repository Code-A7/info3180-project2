...
class TestCreateProfile:
    ...
    def test_create_profile_unauthorized(self, client):
        ...
        response = client.post(
            "/api/profile",
            json={"name": "John", "age": 25, "interests": "hiking, reading, music"},
        )
    ...
    def test_create_profile_insufficient_interests(self, client, verified_user):
        ...
        assert "interests" in response.json.get(
            "errors", {}
        ) or "interests" in response.json.get("errors", {})
    ...
    def test_create_profile_missing_name(self, client, verified_user):
        ...
        response = client.post(
            "/api/profile",
            json={"age": 25, "interests": "hiking, reading, music, gaming, travel"},
            headers={"Authorization": f'Bearer {verified_user["token"]}'},
        )
...
class TestViewOtherProfile:
    ...
    def test_view_other_profile_private_authenticated_owner(
        self, client, app, verified_user
    ):
        ...
        response = client.get(
            f'/api/profile/{verified_user["user_id"]}',
            headers={"Authorization": f'Bearer {verified_user["token"]}'},
        )
...
class TestProfilePictureUpload:
    ...
    def test_upload_picture_success_png(self, client, user_with_profile):
        ...
        data = {"file": (io.BytesIO(b"fake image content"), "test.png")}
    ...
    def test_upload_picture_success_webp(self, client, user_with_profile):
        ...
        data = {"file": (io.BytesIO(b"fake webp content"), "test.webp")}
    ...
    def test_upload_picture_invalid_extension(self, client, user_with_profile):
        ...
        data = {"file": (io.BytesIO(b"fake executable"), "test.exe")}
    ...
    def test_upload_picture_unauthorized(self, client):
        ...
        response = client.post(
            "/api/profile/picture", data=data, content_type="multipart/form-data"
        )
...
class TestProfileValidation:
    ...
    def test_profile_valid_genders(self, client, app, gender):
        ...
        with app.app_context():
            ...
            user = (
                ...
                .filter_by(email=f"gender_test_{gender}@example.com")
                .first()
            )
    ...
    @pytest.mark.parametrize(
        "goal", ["friendship", "casual_dating", "serious_relationship", "marriage"]
    )
    def test_profile_valid_relationship_goals(self, client, app, goal):
        ...
        with app.app_context():
            ...
            user = (
                db.session.query(User)
                .filter_by(email=f"goal_test_{goal}@example.com")
                .first()
            )
            user.is_verified = True
            db.session.commit()
        ...
        response = client.post(
            "/api/auth/login",
            json={"email": f"goal_test_{goal}@example.com", "password": "TestPass123!"},
        )
        token = response.json["token"]
