...
class TestMessaging:
    ...
    def test_send_message_no_match(
    ...
    def test_send_message_success(self, client, match_pair):
        ...
        response = client.post(
            f'/api/messages/{match_pair["user2"]["user_id"]}',
...
class TestTypingStatus:
    ...
    def test_typing_status_typing(self, client, match_pair):
        ...
        response = client.post(
            f'/api/messages/typing/{match_pair["user2"]["user_id"]}',
            json={"is_typing": True},
            headers={"Authorization": f'Bearer {match_pair["user1"]["token"]}'},
        )
        assert response.status_code == 200
    ...
    def test_typing_status_not_matched(
        self, client, user_with_profile, second_user_with_profile
    ):
        """Should return 403 when not matched."""
        response = client.post(
            f'/api/messages/typing/{second_user_with_profile["user_id"]}',
            json={"is_typing": True},
            headers={"Authorization": f'Bearer {user_with_profile["token"]}'},
        )
        assert response.status_code == 403
...
class TestGetMessageHistory:
    ...
    def test_get_message_history_pagination(self, client, match_pair):
        ...
        response = client.get(
            f'/api/messages/{match_pair["user2"]["user_id"]}?page=1&per_page=10',
            headers={"Authorization": f'Bearer {match_pair["user1"]["token"]}'},
        )
        assert response.status_code == 200
    ...
    def test_get_message_history_not_matched(
        self, client, user_with_profile, second_user_with_profile
    ):
        """Should return 403 when not matched."""
        response = client.get(
